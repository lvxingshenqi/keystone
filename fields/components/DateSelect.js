...
module.exports = React.createClass({
	...
	propTypes: {
		...
		optDates: React.PropTypes.array,
	},
	getDefaultProps () {
		return {
			format: 'YYYY-MM-DD',
	...
	getInitialState () {
		const id = ++lastId;
		let month = new Date();
		const { format, value , optDates} = this.props;
	...
	handleSelect (newValue) {
		var value=moment(newValue).format(this.props.format);
		this.props.onChange({ value });
		this.setState({
			pickerIsOpen: false,
			month: moment(newValue,this.props.format).toDate(),
			inputValue: value
		})
	},
	showPicker () {
